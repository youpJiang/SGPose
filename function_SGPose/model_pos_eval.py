...
def rotation_angles(X):
    ...
    # print(euler_angles.shape(name=None))
    # exit()
    return euler_angles
####################################################################
...
def evaluate(data_loader, model_pos_eval, device, summary=None, writer=None, key='', tag='', flipaug='',pad=13,scale=True):
    ...
    for i, temp in enumerate(data_loader):
        ...
        if tag=='3dhp':
            for ii in range(0,len(outputs_3d),30):
                image_name = '/home/jyp/proj/AdaptPose/ketan_viz/3dhp/v5/v5_batch{:0>4d}_{:0>4d}.png'.format(i,ii)
                # image_name = './joint47.png'
                plot_16j(image_name,np.concatenate((outputs_3d[ii:ii+1].numpy(),targets_3d[ii:ii+1].numpy()),axis=0),frame_colors=['r','b'],frame_legend=['pred','gt'])
        else:
            for ii in range(0,len(outputs_3d),30):
                image_name = '/home/jyp/proj/AdaptPose/ketan_viz/h36m/pretrain&GT/pretrain&GT_batch{:0>4d}_{:0>4d}.png'.format(i,ii)
                # image_name = './joint47.png'
                plot_16j(image_name,np.concatenate((outputs_3d[ii:ii+1].numpy(),targets_3d[ii:ii+1].numpy()),axis=0),frame_colors=['r','b'],frame_legend=['pred','gt'])
