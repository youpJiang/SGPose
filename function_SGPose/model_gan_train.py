...
def rotation_angles(X):
    ...
    # e123是以lhip\spin与root节点构建出的互相正交的基底
...
def train_gan(args, poseaug_dict, data_dict, model_pos, criterion, fake_3d_sample, fake_2d_sample, summary, writer,section):
    ...
    for i, ((inputs_3d, _, _, cam_param), target_d2d, target_d3d,target_d3d2) in enumerate(
        ...
        #     os.makedirs('{}/poseaug_viz3'.format(args.checkpoint), exist_ok=True)
        #     image_name1 = '{}/poseaug_viz3/epoch_{:0>4d}_iter_{:0>4d}_inputs_3d_i.png'.format(args.checkpoint, summary.epoch, i)
        #     image_name2 = '{}/poseaug_viz3/epoch_{:0>4d}_iter_{:0>4d}_g_rlt_i.png'.format(args.checkpoint, summary.epoch, i)
        #     image_name3 = '{}/poseaug_viz3/epoch_{:0>4d}_iter_{:0>4d}_2d_i.png'.format(args.checkpoint, summary.epoch, i)
        #     image_name4 = '{}/poseaug_viz3/epoch_{:0>4d}_iter_{:0>4d}_2d——rt_i.png'.format(args.checkpoint, summary.epoch, i)
        #     image_name5 = '{}/poseaug_viz3/epoch_{:0>4d}_iter_{:0>4d}_tar2d_i.png'.format(args.checkpoint, summary.epoch, i)
        #     plot_16j(image_name1,inputs_3d[i:i+1,0,:].cpu().detach().numpy().reshape((-1,16,3)))
        #     plot_16j(image_name2,outputs_3d_rt[i,:].cpu().detach().numpy())
        #     plot_16j_2d(image_name3,inputs_2d[i:i+1,0,pad].cpu().detach().numpy().reshape((-1,16,2)))
        #     plot_16j_2d(image_name4,outputs_2d_rt[i,pad].cpu().detach().numpy().reshape((-1,16,2)))
        #     plot_16j_2d(image_name5,target_d2d[i].cpu().detach().numpy().reshape((-1,16,2)))
