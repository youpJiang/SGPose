...
def main(args):
    ...
    posenet_lr_scheduler = get_scheduler(posenet_optimizer, policy='lambda', nepoch_fix=0,
                                         nepoch=args.epochs,decay_step=args.decay_epoch)
    ...
    for _ in range(start_epoch, args.epochs):
        ...
        if summary.epoch == 0 and not args.debug:
        ...
        for kk in range(5):
            ...
            if summary.epoch > args.warmup:
                ...
                h36m_p1, h36m_p2, dhp_p1, dhp_p2 = evaluate_posenet(args, data_dict, model_pos, model_pos_eval, device,
                                                                    summary, writer, tag='_fake')
                # Update checkpoint
                if not args.debug and (dhpp1_best is None or dhpp1_best > dhp_p1):
                    dhpp1_best = dhp_p1
        ...
        if summary.epoch > args.warmup:
            train_posenet(args.batch_size,model_pos, data_dict['train_fake2d3d_loader'],data_dict['train_gt2d3d_loader'], posenet_optimizer, criterion, device,False)
            h36m_p1, h36m_p2, dhp_p1, dhp_p2 = evaluate_posenet(args, data_dict, model_pos, model_pos_eval, device,
                                                               summary, writer, tag='_real')
        ...
        if not args.debug:
            writer.add_scalar('posenet_mpi3d_loader_flip/lr_now', lr_now, summary.epoch)
            # Update log file
        ...
        summary.summary_epoch_update()
        t2=time.time()
        print('epoch spent:',(t2-t1)/60,'mins')
        print()
